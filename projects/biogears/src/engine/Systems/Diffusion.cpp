...
void DiffusionCalculator::Initialize(SESubstanceManager& subMgr)
{
  for (auto sub : subMgr.GetActiveSubstances()) {
    ...
    if (sub->HasMaximumDiffusionFlux()) {
      ...
      if (sub->GetMolarMass(MassPerAmountUnit::g_Per_mol) < 1000.0 && sub->GetName()!="Bicarbonate") {
        //We elect not to transport HCO3 with simple diffusion because it is a charged substance
