...
void Drugs::CalculatePlasmaSubstanceConcentration()
{
  ...
  for (SESubstance* sub : m_data.GetCompartments().GetLiquidCompartmentSubstances()) {
    ...
    sub->GetAreaUnderCurve().IncrementValue(plasmaConcentration_ug_Per_mL * 1.0e-6 * deltaT_s, TimeMassPerVolumeUnit::s_g_Per_mL);
    ...
    //If we have an antibiotic and an infection present, we need to track how long the antibiotic concentration is above the minimum inhibitory concentration of the bacteria
    if (sub->HasAntibioticPD() && m_data.GetActions().GetPatientActions().HasInfection()) {
		if (plasmaConcentration_ug_Per_mL > m_data.GetActions().GetPatientActions().GetInfection()->GetMinimumInhibitoryConcentration().GetValue(MassPerVolumeUnit::ug_Per_mL)) {
			sub->GetTimeAboveMIC().IncrementValue(deltaT_s, TimeUnit::s);        
		}
    }
