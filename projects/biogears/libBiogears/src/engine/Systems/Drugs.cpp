...
void Drugs::AdministerSubstanceOral()
{
  ...
  for (auto deSub : deactiveSubs) {
    if (oralDoses.at(deSub)->GetAdminRoute() == CDM::enumOralAdministration::Transmucosal) {
      m_TransmucosalStates.erase(deSub);
    }
...
void Drugs::CalculateDrugEffects()
{
  ...
  for (SESubstance* sub : m_data.GetCompartments().GetLiquidCompartmentSubstances()) {
	...
	if (sub->GetClassification() == CDM::enumSubstanceClass::Antibiotic) {
	  //Antibiotic efficacy generally measured by amount of unbound drug
      effectSiteConcentration_ug_Per_mL *= sub->GetPK().GetPhysicochemicals().GetFractionUnboundInPlasma().GetValue();
	  concentrationEffects_unitless = std::pow(effectSiteConcentration_ug_Per_mL, shapeParameter) / (std::pow(ec50_ug_Per_mL, shapeParameter) + std::pow(effectSiteConcentration_ug_Per_mL, shapeParameter)); 
      antibioticEffect_Per_hr += pd.GetAntibacterialEffect(FrequencyUnit::Per_hr) * concentrationEffects_unitless;
    }
  }
